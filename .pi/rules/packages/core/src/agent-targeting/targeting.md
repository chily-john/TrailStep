...
- Custom provider configs require `binary`; optional `model` support uses `{ supported, flag? }`, and optional `thinking` support uses `{ supported, flag?, levels }` with enabled levels from `low`, `medium`, `high`, `xhigh`, or `max`.
