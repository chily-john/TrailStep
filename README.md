...
StepKit supports local TypeScript-authored workflows: packages define workflows with the SDK, expose them from npm-style workflow packages or direct Node-loadable workflow files, discover compatible package exports with the CLI, register convenient refs, and run workflows through the core continuation runtime with persisted run events.
