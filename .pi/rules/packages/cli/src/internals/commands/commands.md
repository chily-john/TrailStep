...
- `trailstep init` writes one or more literal agent entries (starting with `default`) into local, project, or global config through the shared agent setup wizard and can add fully configured custom provider entries from prompts.
