...
Agent mappings control both workflow step dispatch and standalone managed agent sessions opened from the CLI. For workflow steps, TrailStep resolves targets in this order: workflow-level role override, top-level agent with the same role name, top-level size mapping, then `agents.default`. `trailstep` and `trailstep open` launch the first configured default target, `agents.default[0]`, in the current project. Named entries, such as `agents.reviewer[0]`, can be opened with `trailstep open reviewer`; bare `trailstep reviewer` opens the agent only when that token is not also a workflow ref. If a configured agent name matches a provider shortcut, the configured agent wins for open behavior. If a bare token also matches a workflow, TrailStep reports ambiguity and asks for explicit syntax.
