...
- `check-public-package-metadata.mjs`: Update when package set, package names, publish metadata, Changesets config, package files, CLI binary exposure, or workspace/peer dependency invariants change.
...
- If a package metadata, packaging, or artifact-ignore invariant changes intentionally, update the matching verification script in the same change.
