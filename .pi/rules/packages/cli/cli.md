...
- `trailstep update` parses optional scope flags (`--all`, `--workflows`, or `--workflow <name>`), `--force`, and `--yes`/`--assume-yes`; the default self scope reads current TrailStep package ranges, queries npm metadata, scans affected workflow sources for TrailStep deprecations using target TrailStep package versions, then confirms (or uses `--yes`/`--assume-yes`) before rewriting `package.json` and running install, while workflow scopes resolve only registered npm package targets with package metadata, require install-root dependency entries, query latest stable npm metadata from each install root, scan all bundle sources, skip keyword-discovered packages, and skip local-file refs, GitHub refs, stale metadata, or missing package metadata.
