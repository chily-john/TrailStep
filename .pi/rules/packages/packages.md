...
- `create-flows/`: Enter for TrailStep workflow definitions, implementation-doc story splitting with `<!-- trailstep-story-boundary -->`, git-backed story routing/preflight including failed-review retries with clean story boundaries, phase-tracked explore/red/green/validate/review story loops, package workflow manifest wiring, bundled markdown prompt fragments, and build verification for non-TS prompt fragments; currently exposes `takeItAway` and `grillItAway`.
