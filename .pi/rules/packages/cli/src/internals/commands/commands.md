...
- `add/`: Enter when changing `trailstep add` registration of workflow files, bundle workflows, or npm package specs into local/project/global config, `--workflow` single/list/all selection, `--yes` defaulting, scope/namespace/name behavior, conflict handling, or project/user skill generation options.
