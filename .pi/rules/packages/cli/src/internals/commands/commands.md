...
- `update/`: Enter when changing `trailstep update` scope parsing, global CLI target resolution, project package target resolution, workflow package target resolution, npm metadata use, deprecation preflight, or update execution behavior.
...
- `trailstep update` accepts at most one scope flag among `--all`, `--project`, `--workflows`, or `--workflow <name>`; default global CLI scope prints and applies a global `@trailstep/cli` binary update from npm metadata, then refreshes tracked packaged TrailStep usage skill installs when possible, while `--project` is the explicit path for project `@trailstep/core`, `@trailstep/authoring`, and `@trailstep/cli` dependency updates. Project package updates exit no-op with a no-dependencies message when `package.json` has no TrailStep package dependencies, otherwise scan registered/discovered workflows for TrailStep deprecations against target TrailStep package versions, print planned package updates, then require `--yes`, `--assume-yes`, or prompt confirmation before rewriting `package.json` and running install. Workflow scopes resolve only registered npm package targets by package-metadata install root, skip local-file refs, GitHub refs, stale package metadata, and missing package metadata, scan all sources in targeted bundle packages for deprecations, require the target package in the install-root dependencies/devDependencies/peerDependencies, print range-to-version workflow package update plans, use npm metadata from the install root for latest stable dependency updates, and rewrite `package.json`/run install when updates are available. `--all` applies global CLI, project package, and workflow package updates, and uses target TrailStep versions for workflow-package preflight. Bare `--workflow <name>` errors when multiple registrations share that name. Blocking findings stop the update plan unless `--force` is set.
