...
- `launchInteractiveAgentTarget` is the public core helper for standalone built-in/custom interactive target launches; it returns managed-session prompt injection mode and custom providers require `interactiveArgs`.
- Runtime interactive custom commands write `prompt.txt` only when `{{promptFile}}` is used; runtime built-in providers receive the prompt and system prompt file, while standalone launches follow each provider's managed-session prompt delivery spec.
