...
- `remove/`: Enter when changing `trailstep remove` registration deletion, package metadata cleanup, package install preservation notices, or its multi-scope disambiguation.
...
- `trailstep remove` removes package metadata with the registration but does not uninstall workflow packages; it prints a preservation notice when the package remains referenced or the install is not TrailStep-owned.
