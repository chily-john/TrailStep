...
- `interactive-agent/`: Enter when changing inherited-stdio human handoff, standalone interactive target launch, interactive prompt construction, provider target routing, process completion/cancellation, command placeholders, one-off continue Skill prompting, or interactive protocol handoff.
...
- `launchInteractiveAgentTarget` is the public core helper for standalone built-in/custom interactive target launches; it passes target model/thinking through, returns managed-session prompt injection mode, and custom providers require `interactiveArgs`.
- Runtime interactive custom commands write `prompt.txt` only when `{{promptFile}}` is used; runtime built-in providers receive the prompt, system prompt file, and resolved target/role thinking, while standalone launches follow each provider's managed-session prompt delivery spec.
- Runtime interactive provider processes inherit defined process environment values, with `TRAILSTEP_INTERACTIVE_FILE` set to the session protocol file.
