...
Personal collection of TrailStep workflows.
...
trailstep add ./packages/create-flows/src/index.ts --workflow takeItAway
trailstep add ./packages/create-flows/src/index.ts --workflow '*'
trailstep project/take-it-away
```
...
The generated project skills run through `scripts/run-trailstep-isolated.mjs` instead of invoking `trailstep` directly. The wrapper creates `.trailstep/worktrees/<run>/` on a fresh `trailstep/...` branch, copies the input JSON into the worktree, sets `TRAILSTEP_RUNS_ROOT` to the source repo's `.trailstep/runs`, sets `TRAILSTEP_STORY_COMMIT_MODE=enabled`, and runs the workflow there. After each passing story review, the workflow commits the reviewed story diff before the next story starts. With `--pr`, the wrapper pushes the branch and opens a GitHub PR when `gh` is available.
...
trailstep add @trailstep/create-flows --workflow takeItAway
```
