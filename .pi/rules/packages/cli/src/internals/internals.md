...
- `package-manager/`: Enter when changing package-manager detection, add/remove/install command helpers, package.json dependency rewriting, installed TrailStep package version resolution, or npm registry metadata fetching.
...
- `workflow-packages/package-uninstall.ts`: Change when `trailstep remove` package uninstall, preservation, detected package-manager command creation, failure reporting, or package-command invocation changes.
...
- `package-manager/package-manager.ts`: Change when lockfile/packageManager detection, package add/remove command creation, pnpm workspace-root detection, or detected install command execution changes.
...
- Installed package bundle refs can fall back to package-root workflow exports when `trailstep.workflows` is absent; local package bundle refs still require manifest metadata.
...
- Workflow npm/GitHub package installs use the command cwd as dev dependencies for local/project scopes and `~/.trailstep/packages` as dependencies for global scope through the install root's detected package manager, adding pnpm `--workspace-root` at pnpm workspace roots; CLI-run installs record `installOwnership: "trailstep-installed"`, reused existing npm packages record `"reused-existing"`, and GitHub refs must be explicit `github:<owner>/<repo>` refs.
