...
- `~/.trailstep/config.json` contributes run config and user workflow registry entries; use the `homeDir` option on `loadTrailStepProjectConfig`/`loadTrailStepConfig` when command tests need to isolate it. There is no `~/.trailstep/config-local.json` — local overrides only exist at the project scope.
