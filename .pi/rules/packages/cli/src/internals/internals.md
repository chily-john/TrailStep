...
- `workflow-packages/`: Enter when changing npm package spec parsing, scope-aware install roots/save args, or scoped package installation used before `trailstep add` bundle discovery.
...
- Workflow npm package installs use the command cwd with `--save-dev` for local/project scopes and `~/.trailstep/packages` with `--save` for global scope.
