...
- `.trailstep/config.json` and `.trailstep/config-local.json` may register workflow refs as string targets under `workflows`; `workflowMetadata` is CLI registry metadata and should not affect core config validation; expose only project-scope registrations via `loadTrailStepProjectConfig` while passing only core config data to validation.
- `~/.trailstep/config.json` contributes run config and user workflow registry entries; use the `homeDir` option on `loadTrailStepConfig` or `loadTrailStepProjectConfig` when command tests need to isolate it. There is no `~/.trailstep/config-local.json` — local overrides only exist at the project scope.
